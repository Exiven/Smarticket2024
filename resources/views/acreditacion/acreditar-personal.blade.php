    ...
    <div class="row">
      ...
      <label for="dni">*DNI</label>
        <form class="formulariovalidar" id="myform" method="POST">
          ...
          @else
            <input type="text" name="dni" class="form-control" style="margin-right: 20px" value="{{ session('dni') }}" placeholder="INGRESE DNI" autocomplete="off" >
          @endif
          @if(!empty($acreditados->dni_acreditar ))
          ...
          @else
          <label for="nombres">NOMBRES Y APELLIDOS</label>
          <input type="text"  name="nombres" id="idnombre"  class="form-control" value="" placeholder="NOMBRES Y APELLIDOS" oninput="this.value = this.value.toUpperCase();" style="text-transform: uppercase;" autocomplete="off">
          @endif
          </div>        
          <div class="form-group">
            @if(!empty( $acreditados->dni_acreditar ))
