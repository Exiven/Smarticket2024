    ...
    <form class="formularioasignar" id="formularioasignar" action="{{ route('guardar.datos') }}" method="POST">
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-3">
                    <select class="form-control" id="id_evento" name="id_evento" style="border-color: #ff0202;margin-bottom: 15px;">
                      <option selected disabled>ELIJA EVENTO</option>
                        @forelse ($evento as $evento)
                        <option value="{{$evento->cod_evento}}" oninput="this.value = this.value.toUpperCase();" style="text-transform: uppercase;">{{$evento->id}} - {{$evento->nom_evento}}</option>
                        @empty
                        <p>Sin Datos</p>
                        @endforelse
                    </select>
                ...
                <div class="col-3">
                  <div class="col-3"></div>
                    <select class="form-control" id="id_usuario" name="id_usuario" style="border-color: #14e182;margin-bottom: 15px;">
